        ...
        <table class=" w-full overflow-scroll">
            ...
            <tbody>
                <tr class="odd:bg-white even:bg-slate-50" v-for="category in categoryState.categories"
                    ...
                    <td @click="openPopup(category._id,category.name)"><i class="fa fa-pen hover:text-green-500"></i></td>
                    <td @click="deleteCategory(category._id)"><i class="fa fa-trash-can hover:text-red-500"></i></td>
                </tr>
            </tbody>
        </table>
    </div>
    <div id="popup-modal" v-if="popup" class="fixed top-0 left-0 right-0 z-50  p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full justify-center" style="background-color:rgba(0, 0, 0, 0.400)">
        <div class="relative bg-white rounded-lg shadow self-center px-20  max-w-md m-auto">
            <button type="button" @click="closePopup()" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-red-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center" data-modal-hide="popup-modal">
            ...
            <div class="">
                <input id="name" name="name" type="name" v-model="updateCategory.name" autocomplete="name" required=""
                    class="relative block w-full appearance-none rounded-md border border-gray-300 px-3 py-2 text-gray-900 placeholder-gray-500 focus:z-10 focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm mb-2"
                    placeholder="add category" />
            </div>
            <div>
